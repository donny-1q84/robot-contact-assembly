...
def sync_peg_to_hand(
    ...
    """Rigidly follow the hand pose with the physical peg each environment step."""
    ...
    offset_quat = body_pos_w.new_tensor(body_rot_offset).unsqueeze(0).repeat(body_pos_w.shape[0], 1)

    peg_pos_w, peg_quat_w = combine_frame_transforms(body_pos_w, body_quat_w, offset_pos, offset_quat)
