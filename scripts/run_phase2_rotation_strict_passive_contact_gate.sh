...
export RCA_GATE_INSTANCE_NAME="${RCA_GATE_INSTANCE_NAME:-isaac-phase2-rotation-strict-passive-l4}"
...
# Single-hypothesis follow-up to the target-rotation failure:
# - reuse the proven shallow-contact controller
# - keep near-contact orientation hold to avoid the observed branch jump
# - tighten only the success rotation gate from 0.20 rad to 0.18 rad
# - run longer because the previous shallow success terminated at rot=0.1909.
