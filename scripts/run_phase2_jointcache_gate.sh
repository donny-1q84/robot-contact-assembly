...
export RCA_GATE_INSTANCE_NAME="${RCA_GATE_INSTANCE_NAME:-isaac-phase2-jointcache-l4}"
...
export RCA_GATE_EVAL_TIMEOUT_SECONDS="${RCA_GATE_EVAL_TIMEOUT_SECONDS:-720}"
export RCA_SCRIPTED_EVAL_RETRIES="${RCA_SCRIPTED_EVAL_RETRIES:-0}"
export RCA_GATE_STEPS="${RCA_GATE_STEPS:-1200}"
export RCA_GATE_EXTRA_AGENT_ARGS="${RCA_GATE_EXTRA_AGENT_ARGS:---deterministic-reset --socket-pos 0.22,0.04,0.19 --scripted-control-mode joint-ik --joint-ik-step 0.05 --joint-limit-margin 0.02 --abs-control-mode waypoint --abs-pos-step 0.012 --abs-rot-step 0.12 --rotate-control-mode stateful-waypoint --hold-orientation-during-descend --insert-after-alignment --insert-descent-mode joint-cache --insert-vertical-step 0.030 --joint-cache-step 0.035 --joint-cache-step-scale 1.0 --joint-cache-total-limit 1.0 --hold-orientation-during-insert --insert-abort-grace-steps 30 --insert-abort-xy-tol 0.06 --insert-abort-rot-tol 0.65 --stop-on-branch-jump --staged-approach --rotate-before-descend --debug-action-steps 4}"
