...
rca_remote_container_exec "mkdir -p '${REMOTE_VIDEO_DIR}'"
rca_remote_container_exec "cat > '${REMOTE_COMMAND_PATH}' <<'EOF'
/isaac-sim/python.sh scripts/evaluate_rsl_rl_checkpoint.py --task ${TASK_NAME} --headless --video --video_length ${VIDEO_LENGTH} --steps ${VIDEO_LENGTH} --num_envs ${NUM_ENVS} --seed ${SEED} --load_run '${LOAD_RUN}' --checkpoint '${CHECKPOINT}' ${EXTRA_PLAY_ARGS}
EOF"

set +e
rca_remote_repo_exec "set -o pipefail && /isaac-sim/python.sh scripts/evaluate_rsl_rl_checkpoint.py --task ${TASK_NAME} --headless --video --video_length ${VIDEO_LENGTH} --steps ${VIDEO_LENGTH} --num_envs ${NUM_ENVS} --seed ${SEED} --load_run '${LOAD_RUN}' --checkpoint '${CHECKPOINT}' ${EXTRA_PLAY_ARGS} 2>&1 | tee '${REMOTE_LOG_PATH}'"
status=$?
set -e
if [[ ${status} -ne 0 ]]; then
