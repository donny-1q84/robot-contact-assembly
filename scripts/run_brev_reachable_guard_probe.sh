...
DELETE_ON_EXIT="${RCA_BREV_GUARD_DELETE_ON_EXIT:-1}"
KEEP_ON_FAILURE="${RCA_BREV_GUARD_KEEP_ON_FAILURE:-0}"
...
delete_instance() {
  if [[ "${CREATED_INSTANCE}" != "1" || "${DELETE_ON_EXIT}" != "1" ]]; then
    return 0
  fi
...
cleanup() {
  ...
  if [[ "${CREATED_INSTANCE}" == "1" && ( "${KEEP_ON_FAILURE}" != "1" || "${status}" == "0" ) ]]; then
...
[reachable-guard-paid] Set RCA_ALLOW_PAID_BREV_CREATE=1 only after confirming credits/budget and a manual deletion path in the Brev UI.
EOF
  exit 2
fi
