...
RCA_FINAL_CONTACT_TRACE_RUNNER="${SCRIPT_DIR}/run_remote_action_semantics_probe_trace.sh" \
RCA_FINAL_CONTACT_INSTANCE_TYPE="${RCA_ACTION_SEMANTICS_INSTANCE_TYPE:-g6e.xlarge}" \
RCA_FINAL_CONTACT_WATCHDOG_MAX_MINUTES="${RCA_ACTION_SEMANTICS_WATCHDOG_MAX_MINUTES:-30}" \
RCA_FINAL_CONTACT_TRACE_TIMEOUT_SECONDS="${RCA_ACTION_SEMANTICS_TRACE_TIMEOUT_SECONDS:-240}" \
...
RCA_FINAL_CONTACT_WATCHDOG_LEDGER_DIR="${RCA_ACTION_SEMANTICS_WATCHDOG_LEDGER_DIR:-}" \
  "${SCRIPT_DIR}/recreate_brev_and_run_final_contact_servo_trace.sh" \
    "${1:-rca-action-semantics-probe-vm}" \
