...
paid_success_variation_preflight: scripts/check_success_variation_paid_lifecycle_preflight.py summarizes clean source state, current contact-smoke bundle, credit evidence, Brev safety, local-env armability, batch-plan readiness, the pre-batch assumption audit, and the read-only lifecycle_plan without arming or creating a paid instance; the Brev safety summary now exposes status, visible instance count, watchdog process state, and manual-delete alert state; the lifecycle_plan exposes budget, estimated hourly price, watchdog TTL, estimated max cost, manual disarm fallback, safety snapshot command, recovery command, and cleanup guards requiring final SAFE_NO_VISIBLE_PAID_INSTANCE / workspaces null evidence before cleanup can be claimed; blocked_subchecks and unblock_plan expose the concrete credit, local-env, pre-batch audit, and acknowledgement blockers plus the ordered preview/write/arm/preflight/lifecycle commands
