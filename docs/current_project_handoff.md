...
7. if a new Brev instance is required for the batch, use
   ...
   Its dry-run facts list the post-arm cleanup guards. It disarms automatically
   if either local readiness gate fails, if the prepare sequence is interrupted,
   or if an unexpected exception occurs after the local env has been armed.
...
paid_success_variation_preflight: scripts/check_success_variation_paid_lifecycle_preflight.py summarizes clean source state, current contact-smoke bundle, local credit evidence, read-only Brev API credit balance, Brev safety, local-env armability, batch-plan readiness, the pre-batch assumption audit, and the read-only lifecycle_plan without arming or creating a paid instance; api_credit_balance.status must be PASS, so a BLOCKED or UNAVAILABLE API balance fails the real paid entrypoint even if older UI/local evidence looks ready; the Brev safety summary now exposes status, visible instance count, watchdog process state, and manual-delete alert state; the lifecycle_plan exposes budget, estimated hourly price, watchdog TTL, estimated max cost, manual disarm fallback, safety snapshot command, recovery command, and cleanup guards requiring final SAFE_NO_VISIBLE_PAID_INSTANCE / workspaces null evidence before cleanup can be claimed; blocked_subchecks and unblock_plan expose the concrete credit, API-credit, local-env, pre-batch audit, and acknowledgement blockers plus the ordered preview/write/arm/preflight/lifecycle commands
brev_credit_review_packet: scripts/prepare_brev_credit_review.py exposes the Brev org dashboard URL, current credit-evidence blocker, read-only Brev API credit-balance result, credit_consistency UI/API or local-evidence/API consistency status, preview/write-credit commands, preview/real prepare_success_variation_paid_batch.py commands, rerun-preflight command, paid lifecycle command, and the paid preflight blocked_subchecks/unblock_plan without opening paid compute by default; scripts/read_brev_credit_balance.py reads only the current org credit endpoint through local Brev credentials and never prints tokens, payment details, writes evidence, arms env state, or creates instances; if api_credit_balance is BLOCKED, the packet status becomes NEEDS_BREV_CREDIT_TOPUP even if older local/UI evidence would otherwise look ready; if --balance-eur covers the budget while the API remains BLOCKED, credit_consistency.status becomes UI_API_MISMATCH_API_BLOCKED and paid_prepare_allowed=false; if fresh local credit evidence and API credit both pass without a new --balance-eur, credit_consistency.status becomes LOCAL_EVIDENCE_API_CONSISTENT_PASS; pass --balance-eur <current-brev-ui-balance> to replace placeholders with concrete preview commands only after the current API/UI balance is acceptable; write_brev_credit_evidence.py --dry-run and prepare_success_variation_paid_batch.py --dry-run emit parseable facts with no write/arm/create side effects; the real prepare helper now checks Brev API credits before writing local credit evidence or arming the env, then disarms the local env if check-only or aggregate preflight fails, on KeyboardInterrupt after arming, and on unexpected post-arm exceptions
brev_credit_blocker_diagnosis: scripts/diagnose_brev_credit_blocker.py is a read-only bundle for the current credit blocker; it reports active Brev org, visible instance count, CLI version, API credit balance, credit_consistency, likely causes, and safe next steps without opening a browser, writing evidence, arming env state, or creating instances; with --balance-eur 20.00 while API balance is 0, status becomes UI_API_MISMATCH_BLOCKED and paid_prepare_allowed=false
brev_credit_support_packet: scripts/prepare_brev_credit_support_packet.py packages the current credit blocker into local JSON/Markdown plus a support-message draft; it includes expected/active org, API credit balance, UI balance preview, credit_consistency, visible instance count, safe next steps, and exact follow-up commands, while --no-output keeps it fully read-only; it does not write credit evidence, arm env state, create instances, run remote code, start Isaac, copy artifacts, delete instances, or approve bypassing a BLOCKED Brev API credit gate
project_status_report_balance_context: scripts/project_status_report.py accepts --balance-eur <current-brev-ui-balance> and passes that same UI-balance context through the Brev credit review, Brev credit blocker diagnosis, and next-project-action selector; this only makes the read-only report concrete and does not write credit evidence, arm local env state, create paid instances, or override a BLOCKED API credit gate
next_project_action_selector: scripts/select_next_project_action.py is the read-only decision helper for the current phase; it prioritizes Brev credit/API blockers before the fixed-budget success-variation trace batch, then dataset finalization, then policy/API review; current live state selects resolve_brev_credit_blocker with paid_compute_allowed=false
...
external_robot_adapter: configs/v0_external_robot_adapter.template.json is safely blocked by scripts/check_v0_robot_adapter_contract.py; docs/v0_robot_adapter_contract.md defines command/frame/runtime guards plus transfer readiness levels so portability is adapter-specific, direct_use_ready=false, and never drop-in
portability_boundary: scripts/check_v0_portability_boundary.py combines V0 skill readiness with the named adapter contract and keeps universal_drop_in_ready=false plus direct_use_ready=false
portability_review_packet: scripts/prepare_v0_portability_review.py packages the portability boundary, reusable layers, robot-specific layers, current blockers, exact non-drop-in answer, transfer_readiness_level, adapter_workplan, and optional target_adapter_preview into JSON/Markdown without touching Brev, Isaac, ROS, or hardware
