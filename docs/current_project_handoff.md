...
brev_credit_review_packet: scripts/prepare_brev_credit_review.py exposes the Brev org dashboard URL, current credit-evidence blocker, read-only Brev API credit-balance result, preview/write-credit commands, preview/real prepare_success_variation_paid_batch.py commands, rerun-preflight command, paid lifecycle command, and the paid preflight blocked_subchecks/unblock_plan without opening paid compute by default; scripts/read_brev_credit_balance.py reads only the current org credit endpoint through local Brev credentials and never prints tokens, payment details, writes evidence, arms env state, or creates instances; if api_credit_balance is BLOCKED, the packet status becomes NEEDS_BREV_CREDIT_TOPUP even if older local/UI evidence would otherwise look ready; pass --balance-eur <current-brev-ui-balance> to replace placeholders with concrete preview commands only after the current API/UI balance is acceptable; write_brev_credit_evidence.py --dry-run and prepare_success_variation_paid_batch.py --dry-run emit parseable facts with no write/arm/create side effects; the real prepare helper now checks Brev API credits before writing local credit evidence or arming the env, then disarms the local env if check-only or aggregate preflight fails, on KeyboardInterrupt after arming, and on unexpected post-arm exceptions
