...
brev_credit_review_packet: scripts/prepare_brev_credit_review.py exposes the Brev org dashboard URL, current credit-evidence blocker, preview/write-credit commands, preview/real prepare_success_variation_paid_batch.py commands, rerun-preflight command, and paid lifecycle command without opening paid compute by default; write_brev_credit_evidence.py --dry-run and prepare_success_variation_paid_batch.py --dry-run emit parseable facts with no write/arm/create side effects
