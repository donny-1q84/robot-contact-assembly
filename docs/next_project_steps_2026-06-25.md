...
For one combined credit-blocker diagnosis that also checks the active Brev org,
visible instance count, CLI version, API credit, and `credit_consistency`, run:
...
`scripts/run_v0_offline_policy_readiness_pipeline.py` after finalize succeeds,
so a passing fixed-budget batch advances directly into the local policy
readiness chain. It writes a review record even when blocked, and it does not
create or delete Brev instances. The equivalent lower-level commands are:
