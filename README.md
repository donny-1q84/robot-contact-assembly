...
After the repeated Brev create/delete lifecycle stalls, run `scripts/run_brev_probe_only_gate.sh` first. It only creates the selected instance, waits for SSH readiness, probes `nvidia-smi` / disk, and deletes it without installing Isaac or running evaluation. The first `2026-05-21` probe-only run still failed before SSH on `g2-standard-4:nvidia-l4:1`, so do not run another Isaac workload on Brev until probe-only succeeds or a different explicit provider candidate is validated. Non-GCP fallback probes are available as `scripts/run_brev_probe_l40s_nebius_gate.sh` and `scripts/run_brev_probe_l40s_aws_gate.sh`.
